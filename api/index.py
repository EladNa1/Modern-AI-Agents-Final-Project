...
async def agent_info():
    ...
    return {
        "description": (
            ...
            "official solution and rubric, not invented — and one calibration fact is disclosed "
            "up front: the Grader's few-shot examples are calibrated on the same Winter-2024 "
            "Moed-A booklet the bundled samples come from, so the published human-agreement "
            "numbers measure consistency-with-calibration, not held-out generalization (the "
            "few-shot block is stripped when grading any other exam; a held-out third booklet "
            "is the roadmap). Handwriting too ambiguous to grade "
            ...
            "(self-critique that approves, revises, or escalates — and its APPROVEs are proven "
            "agreement, not rubber-stamping: scripts/demo_reflection_teeth.py in the repo seeds "
            "deliberately WRONG grades and records the live Reflector rejecting both, e.g. a "
            "seeded 10/10 on anchored-error work REVISEd to the anchored 7/10; the recorded "
            "trace is eval/examples/05_reflection_teeth.json); the GradeBook assembles the "
            ...
            "rubric. Out-of-domain PROMPTS are refused politely by the Router's scope guard "
            "BEFORE any model call — zero tokens, the refusal logged as a Router decision in "
            "steps[]. An uploaded non-exam IMAGE costs the minimal vision reads that identify "
            "it, after which it is refused at the retrieval threshold without any grading."
        ),
        "purpose": (
