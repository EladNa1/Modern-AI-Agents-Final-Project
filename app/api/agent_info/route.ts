...
export async function GET() {
  ...
  return NextResponse.json({
    description:
      "CheckMate is an autonomous agent that grades Calculus 1 (Hedva 1) exams. You upload a scanned exam (PDF, Word, or images); it reads every page with OCR, splits the exam into questions, and grades each on the student's ACTUAL method — awarding partial credit and writing per-question feedback. Grades are grounded in the retrieved official solution and rubric, not invented. Ambiguous handwriting or low-confidence grades are escalated to a human teacher, never silently guessed. It returns the graded exam with annotations plus the total. Internally it is a Reflection Agent with four logged modules — Parser (vision OCR, splits the exam), Retriever (Pinecone vector RAG that grounds each grade in the matching official solution AND in relevant course lecture notes), Grader (few-shot, partial credit), and Reflector (self-critique that approves, revises, or escalates) — matching the model_architecture diagram. What it CANNOT do: it does not set exam questions, does not tutor, and does not fabricate a rubric.",
    purpose:
